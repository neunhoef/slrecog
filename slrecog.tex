...
of the algorithm, we need the following notation.

Let $q=p^e$ for some prime $p$ and let $(\varepsilon_1, \ldots, \varepsilon_e)$
be a basis of $\GF(q)$ over $\GF(p)$ such that we can easily express
elements of $\GF(q)$ as vectors in this basis in practice. 
For a basis $B=(b_1,b_2,\ldots,b_d)$    
of $V$ and $\alpha \in \GF(q)$, let $E_{ij}(\alpha) \in \SL(V)$ be      
the group element with matrix (in the basis $B$) $M=(m_{k\ell})$ with   
$m_{k\ell}=1$ if $k=\ell$; $m_{ij}=\alpha$; and all other entries of    
$M$ equal to $0$.                                                       

We first need the definition of standard generators for $\SL(d,q)$:

\begin{definition}[Standard generators for $\SL(n,q)$]
Let $G = \left< Y \right>$ be a subgroup of $\GL(d,q)$ that is 
isomorphic to $\SL(n,q)$ and fixes an $d-n$-dimensional subspace $W$ of
$V$. Then $Y$ is called a \emph{set of standard generators for
$\SL(n,q)$ with respect to the basis $(v_1, \ldots, v_d)$} of $V$, 
if it has $2e+2$ elements and contains the following elements:
\begin{itemize}
\item for each $1 \le i \le e$ an element of the form $E_{1,2}(\varepsilon_i)$,
\item for each $1 \le i \le e$ an element of the form $E_{2,1}(\varepsilon_i)$,
...
odd.
\end{itemize}
The $-1$ entries are of course there to ensure that the elements are
contained in $\SL(n,q)$.
\end{definition}
...
program $P$ to a new generating set $Y$ together with a basis
