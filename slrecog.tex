...
%last modified: Max, 22. December 2012
...
For $n' = d < 2n-1$ it is possible that $V_n \cap \Fix c$ has higher 
dimension than $1$.

Let $W := V_n + V_n c$ with $\dim W = n'$. We claim that $W$ is invariant
under the action of $c$.
If $n'=d$ then this is trivial, otherwise we have $n'=2n-1 < d$.
Then we have $V_n + \Fix c = V$, again using Lemma~\ref{dimformula}.
Thus, $W + \Fix c = V$ as well which implies that $\dim(W \cap \Fix c)
= (2n-1)+(d-n+1)-d = n$. However, since $\dim(V_n \cap (W \cap \Fix c))=1$,
we get $V_n + (W \cap \Fix c) = W$. Therefore we can choose a basis
of $W$ as follows: Pick a non-zero vector $v \in V_n \cap \Fix c$, select
$n-1$ vectors from $(v_1, \ldots, v_n)$ to extend this to a basis
of $V_n$ and then choose $n-1$ vectors from $W \cap \Fix c$ to extend this
to a basis of $W$. We immediately read off from this basis that $W$ is
